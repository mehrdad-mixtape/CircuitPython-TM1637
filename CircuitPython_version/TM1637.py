...
# Developed by mehrdad-mixtape https://github.com/mehrdad-mixtape/CircuitPython-TM1637

# This is version for circuitpython 7.2 or higher

# Compatible with 4-Digit-7segment
# TM1637 LED-Driver
# DIO -> GP0 -> pin 1
# CLK -> GP1 -> pin 2
# VCC 3.3v -> pin 36
...
__version__ = '0.1.0v'
__repo__ = 'https://github.com/mehrdad-mixtape/CircuitPython-TM1637'
...
def sleep_us(delay) -> None:
    sleep(delay / 1_000_000)

def sleep_ms(delay) -> None:
    sleep(delay / 1_000) 

class TM1637:
    """Library for quad 7-segment LED modules based on the TM1637 LED driver."""
    def __init__(self, clk: Pin, dio: Pin, brightness=7):
        try:
            self._clk = DigitalInOut(clk)
            self._dio = DigitalInOut(dio)
            self._clk.direction = Direction.OUTPUT
            self._dio.direction = Direction.OUTPUT
        except Exception:
            print('Cannot initialise Digital-pins DIO, CLK')
        else:
